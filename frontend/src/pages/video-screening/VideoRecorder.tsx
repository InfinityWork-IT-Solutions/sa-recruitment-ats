 ...
 * PURPOSE:
 * --------
 * This is the HEART of video screening - the actual video recording component.
 * It handles camera access, recording, playback, and delivers the recorded video
 * back to the parent component.
 * 
 * WHEN IT'S USED:
 * ---------------
 * - Embedded inside VideoRecordingPage.tsx (the main orchestrator)
 * - Used once per question (candidate records 3 separate videos)
 * - Re-instantiated for each question with fresh state
 * 
 * WHAT IT DOES:
 * -------------
 * ✅ Requests camera & microphone permission from browser
 * ✅ Shows live camera preview
 * ✅ Records video using MediaRecorder API
 * ✅ Countdown timer (2:00 → 1:59 → 1:58 ... → 0:00)
 * ✅ Auto-stops recording at 0:00
 * ✅ Shows "REC" indicator during recording
 * ✅ Playback after recording (lets candidate review their answer)
 * ✅ Re-record option (if they're not happy with their answer)
 * ✅ Returns video blob to parent when accepted
 * 
 * USER FLOW:
 * ----------
 * 1. Component mounts → Requests camera permission
 * 2. Camera permission granted → Shows live preview
 * 3. Candidate clicks "Start Recording" → Recording begins, timer starts
 * 4. Candidate speaks for up to 2 minutes
 * 5. Clicks "Stop Recording" OR timer hits 0:00 → Recording stops
 * 6. Video plays back automatically
 * 7. Candidate reviews:
 *    - Not happy? → Click "Re-record" (go back to step 3)
 *    - Happy? → Click "Accept & Continue" (video sent to parent)
 * 
 * BROWSER APIS USED:
 * ------------------
 * navigator.mediaDevices.getUserMedia() - Request camera/mic access
 * MediaRecorder - Record video from camera stream
 * Blob - Store recorded video data
 * URL.createObjectURL() - Create playback URL from blob
 * 
 * PROPS:
 * ------
 * @param maxDuration (number) - Recording time limit in SECONDS (e.g., 120 = 2 minutes)
 * @param onRecordingComplete (function) - Callback when video is accepted
 *                                         Returns: (videoBlob: Blob, videoUrl: string)
 * @param onCancel (optional function) - Callback if user cancels
 * 
 * EXAMPLE USAGE:
 * --------------
 * <VideoRecorder
 ...
 *     // Upload videoBlob to server
 *   }}
 * />
 * 
 * RECORDING SETTINGS:
 * -------------------
 * - Video codec: VP9 (webm format)
 * - Resolution: 1280x720 (720p) ideal
 * - Facing mode: 'user' (front camera on mobile)
 * - Audio: Enabled
 * - Chunk interval: 100ms (collects data every 100ms)
 * 
 * VIDEO OUTPUT:
 * -------------
 * - Format: video/webm
 * - Typical size: 10-15MB for 2 minutes at 720p
 * - Contains: video track + audio track
 * - Returns as: Blob + URL string
 * 
 * ERROR HANDLING:
 * ---------------
 * ❌ Camera permission denied → Shows help instructions
 * ❌ No camera available → Shows error message
 * ❌ Recording fails → Shows retry option
 * ❌ MediaRecorder not supported → Shows browser upgrade message
 * 
 * PERMISSION STATES:
 * ------------------
 * null → Requesting permission (loading)
 * true → Permission granted (show video)
 * false → Permission denied (show error + instructions)
 * 
 * VISUAL INDICATORS:
 * ------------------
 * 🔴 Red dot + "REC" badge → Recording in progress
 * ⏱️ Timer display → Time remaining (turns red when <10 seconds)
 * ▶️ Play button → Click to start recording
 * ⏹️ Stop button → Click to stop recording
 * 🔄 Re-record button → Try again
 * ✅ Accept button → Confirm and continue
 * 
 * MOBILE CONSIDERATIONS:
 * ----------------------
 * ✅ Uses front camera by default (facingMode: 'user')
 ...
 * CLEANUP:
 * --------
 * - Stops camera stream on unmount (prevents battery drain)
 ...
 * - iOS requires user gesture to start recording (no auto-start)
 * - Some Android devices may not support VP9 codec
 * 
 * FUTURE ENHANCEMENTS:
 * --------------------
 * - Add video quality selector (480p/720p/1080p)
 * - Add practice mode (test camera before starting)
 * - Add background blur/virtual backgrounds
 * - Add auto-upload during recording (chunked upload)
 * 
 * ════════════════════════════════════════════════════════════════════════════════
 */
