...
export default function BillingSuccessPage() {
  ...
  // Prefer sessionStorage (set before PayFast redirect) over query param fallback
  const returnTo = sessionStorage.getItem('billing_return_to')
    || searchParams.get('return')
    || '/company/dashboard';

  // Clear it so it doesn't persist for future visits
  sessionStorage.removeItem('billing_return_to');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-green-950 to-slate-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full text-center space-y-6">
        ...
        >
          {returnTo !== '/company/dashboard' ? 'Continue' : 'Go to Your Dashboard'}
