...
def run_migration():
    with engine.begin() as conn:
        ...
        try:
            conn.execute(sa.text("CREATE TYPE userrole AS ENUM ('super_admin', 'agency_admin', 'recruiter', 'hiring_manager')"))
            conn.execute(sa.text("CREATE TYPE jobstatus AS ENUM ('draft', 'active', 'paused', 'closed', 'filled', 'expired')"))
            conn.execute(sa.text("CREATE TYPE employmenttype AS ENUM ('full_time', 'part_time', 'contract', 'temporary', 'internship', 'freelance')"))
            conn.execute(sa.text("CREATE TYPE applicationstatus AS ENUM ('applied', 'screening', 'shortlisted', 'interview_scheduled', 'interviewed', 'offer_pending', 'offer_made', 'offer_accepted', 'hired', 'rejected', 'withdrawn')"))
            conn.execute(sa.text("CREATE TYPE candidatestatus AS ENUM ('active', 'passive', 'placed', 'inactive')"))
        except Exception as e:
            print(f"Enums might already exist: {e}")

        # Create tables
        print("Creating Tables...")
        metadata = sa.MetaData()
        
        agencies = sa.Table('agencies', metadata,
        ...
        users = sa.Table('users', metadata,
            ...
            sa.Column('role', postgresql.ENUM('super_admin', 'agency_admin', 'recruiter', 'hiring_manager', name='userrole'), nullable=False, server_default='recruiter'),
            sa.Column('is_active', sa.Boolean, server_default='true'),
        ...
        client_companies = sa.Table('client_companies', metadata,
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column('agency_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('agencies.id', ondelete='CASCADE'), nullable=False),
            sa.Column('name', sa.String(255), nullable=False),
        ...
        jobs = sa.Table('jobs', metadata,
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column('agency_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('agencies.id', ondelete='CASCADE'), nullable=False),
            sa.Column('client_company_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('client_companies.id', ondelete='SET NULL')),
            ...
            sa.Column('status', postgresql.ENUM('draft', 'active', 'paused', 'closed', 'filled', 'expired', name='jobstatus'), nullable=False, server_default='draft'),
        ...
        candidates = sa.Table('candidates', metadata,
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column('agency_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('agencies.id', ondelete='CASCADE'), nullable=False),
            sa.Column('first_name', sa.String(100), nullable=False),
            sa.Column('last_name', sa.String(100), nullable=False),
            ...
            sa.Column('status', postgresql.ENUM('active', 'passive', 'placed', 'inactive', name='candidatestatus'), server_default='active'),
        ...
        applications = sa.Table('applications', metadata,
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
            ...
            sa.Column('status', postgresql.ENUM('applied', 'screening', 'shortlisted', 'interview_scheduled', 'interviewed', 'offer_pending', 'offer_made', 'offer_accepted', 'hired', 'rejected', 'withdrawn', name='applicationstatus'), nullable=False, server_default='applied'),
        ...
        activities = sa.Table('activities', metadata,
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        ...
        conn.execute(sa.text("CREATE TABLE IF NOT EXISTS alembic_version (version_num VARCHAR(32) NOT NULL, CONSTRAINT alembic_version_pkc PRIMARY KEY (version_num))"))
        conn.execute(sa.text("INSERT INTO alembic_version (version_num) VALUES ('001_initial_schema')"))
